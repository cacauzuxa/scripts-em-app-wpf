...
3. Observe progresso, duração, resultado, exceções e próxima ação durante a execução; valide ETA e ramos somente quando fizerem parte do contrato.
4. Simule os estados aplicáveis: sucesso, pendência, erro, bloqueio e interrupção.
5. Reabra o app e verifique resultado antigo, lock, repetição e limpeza recuperável somente quando esses riscos existirem.
...
9. Se houver operação financeira, aplique também [finance-operations-gate.md](finance-operations-gate.md); carregue `$financial-app-qa` somente se estiver disponível e o fluxo for financeiro.
